...
- `HISTORY_RELEASE_LIMIT`: 预先下载并缓存的历史 Release 数量，默认 `20`，设为 `0` 可关闭历史版本缓存。
...
curl http://127.0.0.1:8787/versions.json
curl http://127.0.0.1:8787/plugins.json
...
`/versions.json` 会列出预先从 GitHub Release 下载到本机的历史版本。服务器会把每个版本的 `apkPath` 转换为 `apkUrl`，并补充 `size` 和 `sha256`：
  ...
  "versions": [
    {
      ...
      "versionCode": 1760000000,
      "versionName": "20260504-abcdef0",
      "releaseTag": "apk-1760000000-abcdef0",
      "apkUrl": "http://server:8787/apk/history/apk-1760000000-abcdef0-amap-companion.apk",
      "sha256": "...",
      "size": 123456,
...
如果需要让客户端绕过本地缓存直连 GitHub，可请求 `/versions-github.json`。

## 10. 插件市场返回格式

更新服务器同时提供 `/plugins.json`。默认读取 `public/plugins.json`，不存在时回退到 `plugins.template.json`。

插件包放在：

```text
update_server/public/plugins/
```
  ...
  "plugins": [
    {
      ...
      "description": "字体、图标、UI 模板和悬浮窗样式示例",
      "path": "/plugins/example.clean.dashboard-2.acplugin"
    }
  ]
}
```

服务器会把 `path` 转换为 `downloadUrl`，并在本地文件存在时自动补充 `size` 和 `sha256`。客户端安装时会校验 SHA-256。
...
```text
update_server/admin_users.json
update_server/plugin_submissions/submissions.json
update_server/plugin_submissions/packages/
update_server/plugin_submissions/blocked-plugins.json
update_server/public/plugins/
update_server/public/plugins.json
```

提交服务只接受 HTTPS 下载地址；本地测试额外允许 `http://127.0.0.1` 和 `http://localhost`。插件包上限为 20MB，扫描通过后仍需管理员批准才会进入市场清单。
